...
let PRON_V2 = {
  pron_v2_by_axios: function (fileAudio) {
    ...
    fetch(pron_v2_url, {
      ...
      .then((data) => {
        try {
          apiSaveSpeakStastics(
            data.status,
            "SightWord1",
            "PRON_V2/PHONEME",
            correctCount + 1,
            quizDataArr[correctCount].Question,
            recordCnt,
            data.data.average_phoneme_score,
            data.data.average_phoneme_score
          );
        } catch (e) {}
        ...
        switch (data.status) {
          case 200:
            // 정상적으로 완료된 경우
            checkScore(data.data);
            break;

          case 401:
            // 인증키가 잘못된 경우
            alert(
              "인증키가 잘못되었습니다. 리딩게이트로 전화주세요. 1599-0533"
            );
            break;
...
let RECORDER = {
  ...
  checkMediaRecorder: function () {
    if (navigator.mediaDevices.getUserMedia) {
      ...
      alert("Record api not supported on your browser! Please use Chrome.");
    }
  },
  ...
  mediaRecorderSuccess: function (stream) {
    ...
    if (MediaRecorder.isTypeSupported("video/webm; codecs=vp9")) {
      ...
      alert(
        "현재 기기에서 지원하는 코덱이 없습니다. 다른 기기로 접속 부탁드립니다."
      );
      return;
    }
    ...
    mediaRecorder.addEventListener(
      "dataavailable",
      RECORDER.mediaRecorderAvailable
    );
    RECORDER.recording();
  },
