...
const delaysec = 700;
const sndGoodJob =
  "https://wcfresource.a1edu.com/newsystem/sound/soundeffect/goodjob.mp3";
const sndLevelUp =
  "https://wcfresource.a1edu.com/newsystem/sound/soundeffect/levelup.mp3";
const sndSuccess =
  "https://wcfresource.a1edu.com/newsystem/sound/soundeffect/success01.mp3";
// 메뉴 토글소리. 교체예정
const sndWheekk =
...
function isSafari() {
  ...
  return (
    detectEnvironment(navigator.userAgent) === "iOS WebView" ||
    detectEnvironment(navigator.userAgent) === "Safari"
  );
}
...
const loadQuizData = async (pStep, pQuizType, fnOnSucc) => {
  await apiQuizData(pStep, pQuizType)
    .then((response) => {
      if (!response.ok) {
...
const saveStatus = async (fnOnSucc) => {
  const user = ssStudyInfo.user;
  if (step >= 1 && step <= 5 && user === "STUDENT") {
    statusCode = "02500" + step;
    let studnEndYn = "N";
    if (
      currentLand == "game" ||
      currentLand == "songandchant" ||
      statusCode == "025005"
    ) {
      studnEndYn = "Y";
      statusCode = "025008";
    ...
    apiSaveRecord(step, studnEndYn)
      ...
      .then((data) => {
        if (!fnOnSucc) {
          stepSaveEbDoDoAbcOnSucc(data.result, data.resultMessage);
        } else {
          fnOnSucc(JSON.parse(data.resultMessage));
        }
      })
      .catch((error) => {
        console.log(error);
...
const goMovie = () => {
  const book = ssStudyInfo.book;
  movieSrc =
    "https://moviebook.a1edu.com/newsystem/moviebook/dodoabc/EBPK" +
    book +
    ".mp4";
...
function dodomodal(image1, image2, image3, callback) {
  ...
  modalContent.style.backgroundImage = `url("${image1}")`;
  modalContent.style.backgroundSize = "contain"; // Changed from 'cover' to 'contain'
  modalContent.style.backgroundRepeat = "no-repeat"; // Prevent image from repeating
  modalContent.style.backgroundPosition = "center"; // Center the background image
  modalContent.style.padding = "20px";
  modalContent.style.border = "none";
  modalContent.style.width = "30%";
  modalContent.style.height = "35%";
  ...
  modalContent.onmouseover = function () {
    modalContent.style.backgroundImage = `url("${image2}")`;
    modalContent.style.cursor = `url("${image3}"), auto`;
  };
  modalContent.onmouseout = function () {
    modalContent.style.backgroundImage = `url("${image1}")`;
...
function dodomodalStart() {
  ...
  var modalContent = document.createElement("div");
  modalContent.style.backgroundImage = `url("../../include/images/btn_start01.png")`;
  modalContent.style.backgroundSize = "contain"; // Changed from 'cover' to 'contain'
  modalContent.style.backgroundRepeat = "no-repeat"; // Prevent image from repeating
  modalContent.style.backgroundPosition = "center"; // Center the background image
  modalContent.style.padding = "20px";
  ...
  // Change background image on mouse over and revert on mouse out
  modalContent.onmouseover = function () {
    modalContent.style.backgroundImage = `url("../../include/images/btn_start02.png")")`;
    modalContent.style.cursor = `url("../../include/images/cursor_hover.png"), auto`;
  };
  modalContent.onmouseout = function () {
    modalContent.style.backgroundImage = `url("../../include/images/btn_start01.png")`;
    //modalContent.style.cursor = 'auto';
...
function dodomodalFinish(callbackLeft, callbackRight) {
  // Create modal elements
  lockScreen(true);
  var modal = document.createElement("div");
  modal.id = "dodomodal";
  ...
  var modalContent = document.createElement("div");
  modalContent.style.backgroundImage = `url("../../include/images/btn_nextbg.png")`; // Button background image
  ...
  buttonAgain.style.backgroundImage = `url("../../include/images/btn_again01.png")`;
  buttonAgain.style.backgroundSize = "contain";
  buttonAgain.style.backgroundRepeat = "no-repeat";
  buttonAgain.style.backgroundPosition = "center";
  buttonAgain.style.width = "40%"; // Adjust width as needed
  buttonAgain.style.height = "100%"; // Adjust height as needed
  buttonAgain.style.marginLeft = "8%";
  buttonAgain.style.cursor = "pointer";

  buttonAgain.style.maxHeight = "150px";
  buttonAgain.style.maxWidth = "150px";

  var buttonNext = document.createElement("div");
  buttonNext.style.backgroundImage = `url("../../include/images/btn_next01.png")`;
  buttonNext.style.backgroundSize = "contain";
  buttonNext.style.backgroundRepeat = "no-repeat";
  buttonNext.style.backgroundPosition = "center";
  buttonNext.style.width = "40%"; // Adjust width as needed
  buttonNext.style.height = "100%"; // Adjust height as needed
  buttonNext.style.marginRight = "8%";
  buttonNext.style.cursor = "pointer";

  buttonNext.style.maxHeight = "150px";
  buttonNext.style.maxWidth = "150px";

  // Add buttons to the button container
  buttonContainer.appendChild(buttonAgain);
  buttonContainer.appendChild(buttonNext);

  // Add button container to modal content
  ...
  buttonAgain.onmouseover = function () {
    buttonAgain.style.backgroundImage = `url("../../include/images/btn_again02.png")`;
    buttonAgain.style.cursor = `url("../../include/images/cursor_hover.png"), auto`;
  };
  buttonAgain.onmouseout = function () {
    buttonAgain.style.backgroundImage = `url("../../include/images/btn_again01.png")`;
  };

  buttonNext.onmouseover = function () {
    buttonNext.style.backgroundImage = `url("../../include/images/btn_next02.png")`;
    buttonNext.style.cursor = `url("../../include/images/cursor_hover.png"), auto`;
  };
  ...
  buttonAgain.onclick = function (event) {
...
const hideDoDoModal = () => {
  var dodomodal = document.getElementById("dodomodal");
  if (dodomodal) {
    dodomodal.remove();
  }
};
