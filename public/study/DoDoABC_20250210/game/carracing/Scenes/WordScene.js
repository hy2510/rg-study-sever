...
import SpeakerButton from "../Objects/Word/SpeakerButton.js";
import Heart from "../Objects/Common/Heart.js";
import ScoreBoard from "../Objects/Common/ScoreBoard.js";
import Track from "../Objects/Word/Track.js";
import Player from "../Objects/Word/Player.js";
import Block from "../Objects/Word/Block.js";
...
export default class WordScene extends Phaser.Scene {
    ...
    init() {
        ...
        this.preBlockIndex = null;
        this.preColorIndex = null;
    ...
    createExample() {
        ...
        const blockColor = this.setBlockColor();
        ...
        this.examples.add(block);

        // 캐릭터 - 블락 충돌 이벤트
        this.physics.add.overlap(this.player, block, this.hitBlock, null, this);
    }
    ...
    hitBlock(player, block) {
        ...
        const blockValue = block.getTextValue();
        block.destroyBlock();

        if (blockValue == this.correctText) {
            this.correctCount++;
    ...
    setLineNum() {
        ...
        if (this.preBlockIndex == null) {
            const randomNum = Phaser.Math.Between(0, 3);
            resultNum = randomNumArr[randomNum];
        }
        else {
            randomNumArr = randomNumArr.filter(el => {
                if (el != this.preBlockIndex) {
                    return true;
                }
            });
        ...
        this.preBlockIndex = resultNum;

        return resultNum;
    }

    setBlockColor() {
        let randomColorArr = ["green", "pink", "yellow", "purple"];
        const randomNum = Phaser.Math.Between(0, 3);

        return randomColorArr[randomNum];
    }
